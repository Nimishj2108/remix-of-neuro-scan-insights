...
export default function Navigation() {
    ...
    const onScroll = () => {
      ...
      if (delta > 0) {
        ...
        if (scrollUpAccum.current > 50 && hidden) {
          setHidden(false);
          scrollUpAccum.current = 0;
          animate(navY, 0, { type: "spring", stiffness: 100, damping: 20 });
        }
      }
  ...
  return (
    ...
    >
      <Link
        to="/"
        className="font-pixel text-xs text-coral glow-text-coral hover:opacity-80 transition-opacity"
      >
        NeuroScan
      </Link>
      ...
      <Link
        to="/"
        className="font-mono text-sm text-coral/60 hover:text-coral transition-colors"
      >
        ← Exit
      </Link>
    </motion.nav>
